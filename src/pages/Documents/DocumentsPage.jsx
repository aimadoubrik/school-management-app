...
const DocumentsPage = () => {
    ...
    useEffect(() => {
        if (user && demandes) {
            const filtered = demandes.filter((demande) => demande.user === user?.name);
                                ...
                                {(documents || []).map((doc) => (
                                    ...
                                    >
                                        <div className="flex items-start gap-3">
                                            <FileText
                                                className={`w-5 h-5 mt-1 ${selectedDocument?.id === doc.id ? 'text-primary' : 'text-base-content/60'
                                                    }`}
                                            />
                                            <div>
                                    ...
                                    <div className="absolute top-0 right-0 p-2">
                                        <span
                                            className={`flex items-center space-x-1 border rounded-full px-2 py-1 ${
                                                {
                                                    'en cours': 'border-yellow-500 text-yellow-500',
                                                    'effectuer': 'border-green-500 text-green-500',
                                                }[demande.status] || 'border-red-500 text-red-500'
                                            } bg-white dark:bg-gray-800`}
                                        >
                                            {{
                                                'en cours': <Clock className="w-4 h-4" />,
                                                'effectuer': <CheckCircle className="w-4 h-4" />,
                                            }[demande.status] || <AlertCircle className="w-4 h-4" />}
