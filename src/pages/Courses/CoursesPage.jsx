...
const CoursesPage = () => {
  ...
  const filteredCourses = courses.filter(
    (c) =>
      ...
      (!selectedTeacher || c.teacherName === selectedTeacher)
  );

  if (status === 'loading')
    return <div className="flex justify-center items-center min-h-screen">Loading...</div>;
  if (status === 'failed') return <div className="text-center text-error">Error: {error}</div>;

  return (
    <div className="min-h-screen bg-base-100 rounded-lg">
            ...
            <div key={course.id} className="card bg-base-100 shadow-xl border border-base-200">
              ...
              <div className="card-body">
                ...
                <p className="flex items-center gap-2">
                  <BookOpen className="w-5 h-5" /> {course.teacherName}
                </p>
                ...
                >
                  ...
                  {course.status === 'completed' ? 'Start Quiz' : 'Start Course'}
                </button>
              </div>
            </div>
          ))
