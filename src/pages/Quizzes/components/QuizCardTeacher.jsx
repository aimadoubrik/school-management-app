    ...
    <div className='card bg-base-200 shadow-lg 
      ...
      <div className="card-body p-4 sm:p-6 flex flex-col justify-between h-full">
        ...
        <div className="min-h-[80px]">
          <h3 className="card-title text-base sm:text-lg md:text-xl mb-2 line-clamp-2" title={quiz.courseName || 'Untitled Quiz'}>
            {quiz.courseName || 'Untitled Quiz'}
          </h3>
          
          <div className={`badge ${timeColor} px-3 py-2 `}>
            {timeStatus}
          </div>
        ...
        <div className="flex-grow space-y-2 text-sm sm:text-base">
          ...
          <div className="flex items-center gap-2 text-base-content/70">
            ...
            <span className="text-base-content truncate" title={new Date(quiz.Deadline).toLocaleString()}>
        ...
        <div className="card-actions justify-between mt-4">
          <button className="btn btn-outline btn-sm text-green-500 hover:text-green-600 transition duration-300 ease-in-out " onClick={() => onViewDetails(quiz.id)}>
            <Eye className="h-4 w-4" />
          </button>
          <button className="btn btn-outline btn-sm text-blue-500 hover:text-blue-600 transition duration-300 ease-in-out" onClick={() => onEdit(quiz)}>
            <Edit className="h-4 w-4" />
          </button>
          <button className="btn btn-outline btn-sm text-red-500 hover:text-red-600 transition duration-300 ease-in-out" onClick={() => onDelete(quiz.id)}>
            <Trash2 className="h-4 w-4" />
          </button>
          <button className="btn btn-outline btn-sm text-purple-500 hover:text-purple-600 transition duration-300 ease-in-out font-semibold" onClick={() => onAddQuestions(quiz.id)}>
